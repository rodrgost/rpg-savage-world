...
export class GeminiAdapter implements Narrator {
  ...
  private buildNarratorSystemPrompt(opts: {
    ...
    const lines = [
      ...
      '- ACTIONTYPE RULE (mutually exclusive — NO exceptions): diceCheck.required=true and actionType="custom" can NEVER coexist. If required=true, actionType MUST be "trait_test" (put "skill" or "attribute" in actionPayload), or "attack"/"heal" when appropriate — even if the action felt generic/custom. SELF-CHECK each option before emitting: if required=true ⇒ actionType ∈ {trait_test, attack, heal}; if you cannot justify a trait_test, set required=false and keep custom. "custom" + required=true is INVALID and triggers a system promotion that may lose payload data.',
      '',
      ...
      '  ATENÇÃO: "custom" é para ações resolvidas pela narrativa, e por isso tem required: false.',
      '  Se a ação EXIGE um teste (Percepção, Pesquisa, Ocultismo, Furtividade, Atletismo etc.), ela deixa de ser "custom": emita-a como actionType "trait_test" com required: true e a skill/attribute no actionPayload. NUNCA combine "custom" com required: true.',
      '',
