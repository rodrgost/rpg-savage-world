...
// ─── Dificuldade situacional → modificador fixo ───
//
// O LLM informa apenas um rótulo de dificuldade (sinal de ficção limitado); o
...
export function difficultyToModifier(difficulty: string | null | undefined): number {
  if (typeof difficulty !== 'string') return 0
  const key = normalizeSkillLookupValue(difficulty) as DifficultyLabel
  return DIFFICULTY_MODIFIER[key] ?? 0
}
...
/**
 * Resolve a definição de arma a partir de uma chave, rótulo ou nome livre de
 * item de inventário. Faz match exato e, em seguida, por inclusão de substring
 * (ex.: "Pistola 9mm" → "Pistola"), preferindo o rótulo mais longo para evitar
 * que "Espada" capture antes de "Espada Curta".
 */
export function findWeaponDefinition(nameOrKey: string | null | undefined): WeaponDefinition | undefined {
